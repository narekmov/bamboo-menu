...
import XIcon from '../../assets/icons/x_button.svg';

export const Order = () => {
  const {
    orders = [],
    language,
  } = useSelector(({ order, language }) => ({
    orders: order.orders,
  ...
  const orderItem = useCallback(({ item, index }) => (
    <View style={styles.orderItemWrapper} key={item._id.toString()}>
      <TouchableOpacity style={orderGrid.icon} onPress={() => removeHandler(index)}>
        <XIcon />
      </TouchableOpacity>
      ...
      <View style={orderGrid.quantity}>
        <Text style={[styles.orderItemText, styles.textCenter]}>{item.quantity}</Text>
      </View>
      <View style={orderGrid.time}>
        <Text style={[styles.orderItemText, styles.textCenter]}>{item.time ? item.time : '-'}</Text>
      </View>
      <View style={orderGrid.price}>
        <Text
          style={[styles.orderItemText, styles.textEnd]}>{item.price}
  ...
  const OrderTitle = useCallback(() => (
    <View style={styles.orderItemWrapper}>
      ...
      <View style={orderGrid.quantity}>
        <Text style={[styles.orderTitleText, styles.textCenter]}>Quantity</Text>
      </View>
      <View style={orderGrid.time}>
      ...
      <View style={orderGrid.price}>
        <Text
          style={[styles.orderTitleText, styles.textEnd]}>Price</Text>
      </View>
    </View>
  ));
  ...
  const OrderFooter = useCallback(() => (
    <View style={styles.orderFooter}>
      <View style={styles.orderTotalWrapper}>
        ...
        <Text style={styles.orderTotalPrice}>{orders.reduce((s, p) => s + p.price, 0)}
          <Text style={styles.orderCurrency}> AMD</Text>
        </Text>
      </View>
      <TouchableOpacity style={styles.orderButton} onLongPress={removeAllOrders}>
